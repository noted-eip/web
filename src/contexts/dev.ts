...
export const addAccountToDevelopmentContext = (id: string, token: string, setAccounts: React.Dispatch<React.SetStateAction<TAccountsMap>>) => {
...
export const removeAccountFromDevelopmentContext = (id: string, setAccounts: React.Dispatch<React.SetStateAction<TAccountsMap>>) => {
  return setAccounts((old: TAccountsMap) => {
    const ret = {...old}
    ret[id] = undefined
    window.localStorage.setItem(LS_DEVELOPMENT_DATA_KEY, JSON.stringify(ret))
...
export const DevelopmentContext = React.createContext<TDevelopmentContext | undefined>(undefined)
